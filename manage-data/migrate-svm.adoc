...
Before you migrate a storage virtual machine (VM) from an ASA cluster to an ASA r2 cluster, you must run a migration pre-check and perform any required remediation. You cannot perform the migration until the pre-check is passed successfully.
...
----
+
If any remediation is necessary to prepare your ASA cluster for migration, the issue and the corrective action is displayed.   Perform remediation and then repeat the pre-check until it completes successfully.
...
If you disabled automatic cutover and automatic storage VM removal, you should manually perform the cutover of your ASA clients to your ASA r2 cluster and manually remove the storage VM from the ASA cluster.

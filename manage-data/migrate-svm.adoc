...
SVM migration is supported as follows:

[cols="2", options="header"]
|===
| From any of the following ASA platforms: | To any of the following ASA r2 platforms:
a|
* ASA C800
* ASA C400
* ASA C250
...
[NOTE]
For the most current list of ASA and ASA r2 platforms, see link:https://hwu.netapp.com/[NetApp Hardware Universe^]. ASA r2 platforms are listed in NetApp Hardware Universe as "ASA A-Series/C-Series (New)".
...
+
Use the `vserver nvme namespace show` to command verify if there are any NVMe namespaces present.
** Each volume should contain a single LUN.
+ 
Use the `lun show` command to verify the number of LUNs present in each volume.

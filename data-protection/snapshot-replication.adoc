...
For added security against ransomware attacks, create a SnapLock for SnapVault replication relationship.
...
Protect your data by replicating snapshots to a remote cluster.  Replicated snapshots of your consistency groups can be used for disaster recovery or for data migration.
...
. Search for and select the source consistency group.
. Under *Replication destination*, select the cluster to replicate to, and then select the storage VM.

// table end
|===

. Select a replication policy.
. To delay the start of the data transfer, select *Destination settings*, and then deselect *Start transfer immediately*.
+ 
The initial data transfer begins immediately by default.  
. Optionally, to override the default transfer schedule, select *Destination settings*, and then select *Override transfer schedule*.
+
...
For protection against ransomware attacks, when you set up your replication relationship, you can enable SnapLock for SnapVault by selecting a vault replication policy, and then selecting to lock the destination snapshots. Snapshots protected by SnapLock for SnapVault cannot be deleted accidentally or maliciously. 

NOTE: ASA r2 systems do not support SnapLock Compliance mode or SnapLock Enterprise mode.  Creating a SnapLock for SnapVault replication relationship only enables the locking of destination snapshots. It does not enable SnapLock Compliance or Enterprise mode.

.Before you begin

You must link:../secure-data/snaplock.html#initialize-the-snaplock-compliance-clock[initialize the SnapLock Compliance Clock].

.Steps

. In System Manager, select *Protection > Consistency groups*, and then select a consistency group.
...
. Select *Destination settings*, and then select *Lock destination snapshots to prevent deletion*.
...
Now that you have created a replication relationship, your initial data transfer begins as defined in your replication policy.  You can optionally test your replication failover to verify that successful failover can occur if your ASA r2 system goes offline.

...
== Create a SnapLock for SnapVault replication relationship
For protection against ransomware attacks, when you set up your replication relationship, you can enable SnapLock for SnapVault by selecting the a vault replication policy and then selecting to lock the destination snapshots. Snapshots protected by SnapLock for SnapVault cannot be deleted accidentally or maliciously. 
...
You must link:../secure-data/ransomware-protection.html#initialize-the-snaplock-compliance-clock[initialize the Snapshot Compliance Clock].

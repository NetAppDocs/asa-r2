...
Beginning with ONTAP 9.17.1, you can apply asynchronous replication policies directly to child consistency groups.  In ONTAP 9.16.1, replication is supported only at the parent level, meaning all child consistency groups share the replication policy applied to the parent.

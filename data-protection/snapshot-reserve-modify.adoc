...
.Steps
. In System Manager, select *Cluster > Storage VMs*; then select *Settings*.
. Under *Policies*, next to *Snapshots*, select image:icon_kabob.gif[three vertical dots] ; then select *Set/edit snapshot reserve default*.
. Under *Snapshot reserve %*, enter the numeric value for the percentage of the storage unit’s space that you want allocated to snapshots.
. Verify that *Automatically delete older snapshots* is selected.
. Select *Save*.

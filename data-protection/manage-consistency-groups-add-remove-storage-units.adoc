...
Expand the amount of storage managed by a consistency group by adding storage units to the consistency group. You can add existing storage units to your consistency group or you can create new storage units to add to the consistency group.

Beginning with ONTAP 9.18.1, you can set snapshot reserve and automatic snapshot deletion to limit the amount of space used by snapshots in your storage units.  When you add a storage unit to an existing consistency group, snapshot reserve and automatic snapshot deletion are set as follows:

[cols="3", options="header"]
|===
| If you add...
| The snapshot reserve percentage is set to...
...
The default settings for new storage units can be modified when the storage units are created.  You can also link:snapshot-reserve-modify.html[modify existing storage units] to update their current settings. 

...
.Before you begin
If you are running ONTAP 9.16.1 and the consistency group you want to expand is in an SnapMirror active sync relationship, you must link:snapmirror-active-sync-delete-relationship.html[delete the SnapMirror active sync relationship] before you can add storage units. Deleting this relationship before adding storage units from a consistency group is not required in ONTAP 9.17.1 and later releases.

...
Beginning in ONTAP 9.16.1, you can use System Manager to create hierarchical consistency groups on your ASA r2 system.  In an hierarchical structure, one or more consistency groups are configured as children under a parent consistency group. The parent consistency group is managed as a single unit and the child consistency groups are managed as separate units.  This simplifies data protection and management for complex data structures.    

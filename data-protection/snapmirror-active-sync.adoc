...
To link:snap-mirror-active-sync-prepare.html[prepare to configure SnapMirror active sync] on your ASA r2 system you should review the configuration prerequisites, confirm support for your host operating systems, and be aware of object limits that might impact specific configuration.
...
[role="quick-margin-para"]
After installing or configuring the mediator, you must link:initialize-ontap-mediator.html[initialize it] before you can begin using it with SnapMirror active sync for cluster monitoring.

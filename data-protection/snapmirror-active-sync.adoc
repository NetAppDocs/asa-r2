...
[NOTE]
Beginning with ONTAP 9.19.1 RC, ARP/AI supports primary read-write storage units in SnapMirror active sync SAN relationships on ASA r2 systems. ARP/AI runs on active primary storage units and creates primary-only ARP snapshots. For ARP/AI enablement steps, see link:../secure-data/enable-anti-ransomware-protection.html[Enable autonomous ransomware protection with AI].

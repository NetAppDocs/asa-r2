...
[NOTE]
Beginning with ONTAP 9.19.1 RC, ARP/AI supports primary read-write volumes in SnapMirror active sync SAN relationships on ASA r2 systems. ARP/AI runs on active primary volumes, creates primary-only ARP snapshots, and provides machine-learning-based ransomware detection for SAN storage units hosted on those volumes. For ARP/AI enablement steps, see link:../secure-data/enable-anti-ransomware-protection.html[Enable autonomous ransomware protection with AI].

...
[role="quick-margin-para"]
You can use ONTAP Mediator or ONTAP Cloud Mediator to monitor the health of your cluster and enable business continuity.  If you are using ONTAP Mediator, you must link:install-ontap-mediator.html[install it].  If you are using ONTAP Cloud Mediator, you can skip this step.  
...
[role="quick-margin-para"]
After installing or configuring Mediator, you must link:initialize-ontap-mediator.html[initialize it] before you can begin using it with SnapMirror active sync for cluster monitoring.

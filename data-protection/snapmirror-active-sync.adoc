...
---
= SnapMirror active sync setup workflow for ASA r2 systems
:icons: font
:imagesdir: ../media/
...
While the System Manager procedures for configuring SnapMirror active sync are different on ASA r2 systems than NetApp FAS, AFF, and ASA systems running the unified ONTAP personality, the requirements, architecture and operation of SnapMirror active sync is the same. link:https://www.netapp.com/pdf.html?item=/media/138366-sb-3457-san-disaster-recovery-netapp-asa.pdf[Learn more about disaster recovery with SnapMirror active sync on your ASA r2 system^]

On ASA r2 systems, SnapMirror active sync supports symmetric active/active configurations. In a symmetric active/active configuration, both sites can access local storage for active I/O. Learn more about link:https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/architecture-concept.html#symmetric-activeactive[symmetric active/active configurations^].

SnapLock Enterprise and Compliance modes are not supported with SnapMirror active sync on ASA r2 systems.

.About this task

* Beginning with ONTAP 9.19.1, Autonomous Ransomware Protection with Artificial Intelligence (ARP/AI) supports primary read-write storage units in SnapMirror active sync SAN relationships on ASA r2 systems. ARP/AI runs on the active primary storage units, creates primary-only ARP snapshots, and provides machine-learning-based ransomware detection for those storage units. For ARP/AI enablement steps, see link:../secure-data/enable-anti-ransomware-protection.html[Enable Autonomous Ransomware Protection with AI].
...
If you are running ONTAP 9.19.1 or later and enabling SnapMirror active sync for an AIX host, you should link:prepare-aix-snapmirror-active-sync.html[prepare the AIX host]. 

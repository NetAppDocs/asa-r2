...
Instead of traditional aggregates, which own a specific set of the disk available in a storage system, ASA r2s uses a single _storage pod_.  A storage pod is a common pool of storage that has access to all available disks in the storage system.  The storage pod is visible to both nodes in an ASA r2 HA pair.   When a storage unit (LUN or NVMe namespace) is created, ONTAP automatically creates a volumes containing a storage virtual machine (VM) in the storage pod to house the storage units.
...
The following commands and command sets are not supported on ASA on r2s:

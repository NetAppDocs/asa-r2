...
+
Automatic failover is enabled by default on iSCSI only LIFS.  In ASA r2 systems, the default networking LIF is shared between NVMe and SCSI hosts, so doesn't support automatic failover. link:https://docs.netapp.com/us-en/ontap/san-admin/asa-iscsi-lif-fo-task.html#enable-automatic-iscsi-lif-failover[Create an iSCSI only LIF to enable automatic iSCSI LIF failover^].  
+

...
link:https://docs.netapp.com/us-en/ontap/networking/configure-tls-offload.html[Learn to configure TLS hardware offload^].
...
Learn more about common performance features supported on ASA r2 systems.
...
link:https://docs.netapp.com/us-en/ontap/performance-admin/guarantee-throughput-qos-task.html[Learn more about QoS throughput ceilings^].
...
link:https://docs.netapp.com/us-en/ontap/networking/improve-tcp-wan-performance.html[Learn more about improving TCP WAN performance with PRR and RACK].
...
Beginning with ONTAP 9.19.1, an FC switch is not required to establish connections between FC and FC-NVMe hosts and ONTAP storage.  Hosts can be directly connected to FC adapter ports on ASA r2 systems.  Direct-attachment capability can be especially valuable in remote sites and hybrid environments where simplicity is key. 

* link:https://docs.netapp.com/us-en/ontap/san-config/configure-fc-nvme-hosts-ha-pairs-reference.html#direct-attach-fc-and-fc-nvme-configurations[Learn more about SAN direct attachment^].
...
== Storage efficiency
Storage efficiency is the measure of how effectively a storage system uses available space by optimizing storage resources, minimizing wasted space, and reducing written data's physical footprint. 

ONTAP provides various storage efficiency technologies that can help you optimize storage usage and reduce costs.

link:https://docs.netapp.com/us-en/ontap/concepts/storage-efficiency-overview.html[Learn more about ONTAP storage efficiency technologies].

=== Global deduplication

Beginning with ONTAP 9.19.1, _global deduplication_ extends the link:https://docs.netapp.com/us-en/ontap/concepts/deduplication-concept.html[existing ONTAP deduplication space savings^] at the storage unit level to the entire Storage Availability Zone (SAZ). By eliminating duplicate data blocks across all the storage units in the SAZ, storage efficiency can be significantly improved and total cost of ownership reduced. Global deduplication requires no feature license, is enabled by default, integrates seamlessly with existing storage efficiency technologies, and operates transparently with minimal performance impact and without the need for administrative configuration. 
...
// 2026, May 07, ONTAPDOC-3498, ONTAPDOC-3677, ONTAPDOC-3883
...
=== Quality of Service (QoS) throughput ceiling increases

A QoS maximum or throughput ceiling limits throughput for a workload to a maximum number of IOPS or MBs per second, or IOPS and MBs per second. Set a throughput ceiling on a workload to limit its impact on system resources and prevent it from negatively affecting more critical workloads. 

You can modify the default settings that control how QoS policy groups automatically increase the QoS throughput ceiling in response to short-term needs for higher performance. These settings cannot be modified in ONTAP 9.18.1 and earlier. Adaptive and non-adaptive QoS policy groups are automatically enabled to temporarily increase a workload’s throughput ceiling by 50% for a duration of 1 second to respond to short-term demands for higher performance by default. Beginning with ONTAP 9.19.1, you can modify these default settings.  In ONTAP 9.18.1 and earlier these settings cannot be changed.
...
// 2026, May 07, ONTAPDOC-3498, ONTAPDOC-3677
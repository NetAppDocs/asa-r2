...
* ARP/AI is not supported on storage units protected by SnapMirror synchronous or SnapLock.
...
If you are running ONTAP 9.17.1, you can enable ARP/AI on all storage units created in the cluster by default. In ONTAP 9.18.1 and later, ARP/AI is enabled by default on all new and existing clusters. You need to enable it only if you disabled the default enablement.

.Steps
. In System Manager, select *Cluster > Settings*.
. Next to *Anti-ransomware*, select image:icon_kabob.gif[Edit anti-ransomware settings] and then select *Enable on all existing storage units*.
. Select *Enable*.

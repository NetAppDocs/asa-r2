...
If you need to switch from the onboard key manager to an external key manager or vice versa, see link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/migrate-keys-between-key-managers[Migrate ONTAP data encryption keys between key managers^].

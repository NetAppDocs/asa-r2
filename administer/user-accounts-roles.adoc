...
An Lightweight Directory Access Protocol (LDAP) server enables you to centrally maintain user information. If you store your user database on an LDAP server in your environment, you can configure your storage system to look up user information in your existing LDAP database.
...
+
To add multiple attributes, select image:icon_add.png[plus sign followed by the word add in blue letters].
. Select *Save*.
...
. To enable MFA, select image:icon_add.png[plus sign followed by the word add in blue letters]; then select a secondary login method and authentication method 

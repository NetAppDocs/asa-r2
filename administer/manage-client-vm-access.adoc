...
A LIF (network interface) is an IP address associated with a physical or logical port.  Create LIFs on the ports you want to use to access data. Storage VMs serve data to clients through one or more LIFs. On an ASA r2 system, you can create IP, FC, and NVMe/FC LIFs.  An IP data LIF can service both iSCSI and NVMe/TCP traffic by default.  Separate data LIFs must be created for FC and NVMe/FC traffic.

.LIF failover on ASA r2 systems

If there is a component failure, a LIF can fail over or be migrated to a different physical port, so that network communication is not interrupted. Beginning with ONTAP 9.19.1, automatic LIF failover is supported for IP LIFs by default. When automatic LIF failover is enabled, if a storage failover occurs, the LIF is automatically migrated from its home node or port to its HA partner node or port and then back once the failover is complete. Or, if the port becomes unhealthy, the LIF is automatically migrated to a healthy port in its current home node and then back to its original port once the port is healthy again.

In ONTAP 9.18.1 and earlier, automatic LIF failover is not supported for IP LIFs with both iSCSI and NVMe/TCP traffic. When you create iSCSI-only LIFs, automatic LIF failover is enabled by default unless the storage VM already has iSCSI LIFs that do not have automatic LIF failover enabled.  If there are iSCSI LIFs on the storage VM that are not enabled for automatic LIF failover, your newly created LIFs will not be enabled either. In such cases, you must first delete the existing iSCSI LIFs and then recreate them with automatic LIF failover. Learn how to link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html[delete a LIF in a SAN environment^].

Persistent ports are used for failover of FC and NVMe/FC LIFs. Learn more about link:https://docs.netapp.com/us-en/ontap/san-config/asa-support-limitations.html#support-for-persistent-ports[persistent ports^] in the ONTAP documentation.
...
If you want to enable automatic iSCSI LIF failover in ONTAP 9.18.1 or earlier, you must first create a custom iSCSI-only LIF service policy.  
...
In ONTAP 9.18.1 and earlier, automatic LIF failover is enabled by default when you create iSCSI-only LIFs if all other iSCSI LIFs on the storage VM are also enabled for automatic LIF failover. If there are iSCSI LIFs on the storage VM that are not enabled for automatic LIF failover, your newly created LIFs will not be enabled for automatic LIF failover either. 
...
You must use the ONTAP command line interface (CLI) to create an iSCSI-only LIF.
...
* link:https://docs.netapp.com/us-en/ontap/san-admin/move-san-lifs-task.html[Move a SAN LIF^].
* link:https://docs.netapp.com/us-en/ontap/san-admin/remove-lif-from-portset-task.html[Remove a SAN LIF from a portset^].

// 2026, Aug 07, ONTAPDOC-3890/3573

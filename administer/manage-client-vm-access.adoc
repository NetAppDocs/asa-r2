...
Beginning with ONTAP 9.19.1, default automatic LIF failover is supported for iSCSI, NVMe/TCP and shared IP LIFs. When automatic LIF failover is enabled, if a storage failover occurs, the LIF is automatically migrated from its home node or port to its HA partner node or port and then back once the failover is complete. Or, if the port becomes unhealthy, the LIF is automatically migrated to a healthy port in its current home node and then back to its original port once the port is healthy again.

In ONTAP 9.18.1 and earlier, automatic LIF failover is not supported for NVMe/TCP LIFs or for shared IP LIFs. It is enabled by default when you create iSCSI-only LIFs if there are no pre-existing iSCSI LIFs on the storage VM that are not enabled for automatic LIF failover. If there are iSCSI-only LIFs on the storage VM that are not enabled for automatic LIF failover, your newly created LIFs will not be enabled either. In such cases, you must first delete the existing iSCSI-only LIFs and then recreate them with automatic LIF failover.

Persistent ports are used for failover of FC and NVMe/FC LIFs.

...
If there is a component failure, a LIF can fail over or be migrated to a different physical port, so that network communication is not interrupted. Beginning with ONTAP 9.19.1, automatic LIF failover is supported for iSCSI, NVMe/TCP and shared IP LIFs by default. When automatic LIF failover is enabled, if a storage failover occurs, the LIF is automatically migrated from its home node or port to its HA partner node or port and then back once the failover is complete. Or, if the port becomes unhealthy, the LIF is automatically migrated to a healthy port in its current home node and then back to its original port once the port is healthy again.

In ONTAP 9.18.1 and earlier, automatic LIF failover is not supported for NVMe/TCP LIFs or shared IP LIFs. When you create iSCSI-only LIFs, automatic LIF failover is enabled by default unless the storage VM already has iSCSI LIFs that do not have automatic LIF failover enabled.  If there are iSCSI-only LIFs on the storage VM that are not enabled for automatic LIF failover, your newly created LIFs will not be enabled either. In such cases, you must first delete the existing iSCSI-only LIFs and then recreate them with automatic LIF failover. Learn how to link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html[delete a LIF in a SAN environment^].

...
+
When you specify the IP address range for the subnet, do not overlap IP addresses with other subnets.  If IP addresses overlap, network issues can occur if different subnets or hosts attempt to use the same IP address.
...
. Select *Network interfaces*; then select image:icon_add_blue_bg_png[blue rectangle with a plus sign followed by the word add in white letters].

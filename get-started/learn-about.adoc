...
ASA r2 systems support a single HA-pair deployment using a symmetric active-active architecture for multipathing so that all paths between the hosts and storage are active/optimized.  All SAN protocols (iSCSI, FC, NVMe/FC and NVMe/TCP) are supported.
